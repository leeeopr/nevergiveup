...
export default function Configuracoes() {
  const [settings, setSettings] = useState(() => {
    if (typeof window !== "undefined") return loadFinancialData().settings;
    return { notificationEmail: "", startDate: new Date().toISOString().split('T')[0] };
  });

  useEffect(() => {
    if (typeof window !== "undefined") {
      const timer = setTimeout(() => {
        setSettings(loadFinancialData().settings);
      }, 0);
