...
import { Category, FinancialData, CategoryType } from "@/lib/types";
...
export default function Categorias() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newId = editingCategory?.id || `cat_${Date.now()}`;
    ...
    const newData = { ...data };
    if (editingCategory) {
      const index = newData.categories.findIndex(c => c.id === editingCategory.id);
      if (index !== -1) {
